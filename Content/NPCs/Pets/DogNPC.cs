...
namespace zhashi.Content.NPCs
{
    public class DogNPC : ModNPC
    {
        ...
        public override void SendExtraAI(BinaryWriter writer) { writer.Write(isStaying); writer.Write(attackState); writer.Write(stateTimer); writer.Write(OwnerName ?? ""); writer.Write(currentPathway); writer.Write(currentSequence); writer.Write(MyName ?? "旺财"); writer.Write(BonusMaxHP); writer.Write(isInitialized); }
        public override void ReceiveExtraAI(BinaryReader reader) { isStaying = reader.ReadBoolean(); attackState = reader.ReadInt32(); stateTimer = reader.ReadInt32(); OwnerName = reader.ReadString(); currentPathway = reader.ReadInt32(); currentSequence = reader.ReadInt32(); MyName = reader.ReadString(); BonusMaxHP = reader.ReadInt32(); isInitialized = reader.ReadBoolean(); }
