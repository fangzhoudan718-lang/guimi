...
namespace zhashi.Content.Buffs.Debuffs
{
    public class ExtremeBadLuckBuff : ModBuff
    {
        ...
        {
            float activity = player.GetModPlayer<SealedArtifactActivityPlayer>().Activity;
            bool authoritative = Main.netMode != NetmodeID.MultiplayerClient;
            // =========================
            // 下面全是由于运气差导致的物理后果
            ...
            // 1. 属性小幅削弱 (精神不振，手感变差)
            player.statDefense -= activity >= 35f ? 5 : 2;
            player.endurance -= activity >= 70f ? 0.1f : 0.04f;
            player.GetDamage(DamageClass.Generic) *= activity >= 70f ? 0.85f : 0.95f;
            player.pickSpeed += activity >= 35f ? 0.2f : 0.08f;

            // 2. 视觉折磨 (瞎眼)
            if (activity >= 35f) player.AddBuff(BuffID.Darkness, 2);
            if (activity >= 70f) player.AddBuff(BuffID.Blackout, 2);
            if (activity >= 35f && Main.rand.NextBool(12))
                Dust.NewDust(player.position, player.width, player.height, DustID.Smoke, 0, -2, 100, Color.Black, 2f);

            // 3. 漏财
            if (authoritative && activity >= 35f && player.velocity.Length() > 0.1f && Main.rand.NextBool(900))
            {
            ...
            // 4. 平地摔
            if (authoritative && activity >= 55f && player.velocity.Y == 0 && System.Math.Abs(player.velocity.X) > 2f && Main.rand.NextBool(1200))
            {
                player.velocity.X = 0;
            ...
            // 5. 天灾 (雷劈/流星)
            if (authoritative && activity >= 75f && !Main.dayTime && player.ZoneOverworldHeight && Main.rand.NextBool(1800))
            {
                ...
                int p = Projectile.NewProjectile(player.GetSource_Buff(buffIndex), spawnPos, velocity, ProjectileID.FallingStar, 160, 10f, player.whoAmI);
            ...
            // 6. 武器反噬 (炸膛 / 割伤)
            if (authoritative && activity >= 70f && player.itemAnimation > 0 && !player.HeldItem.IsAir && player.HeldItem.damage > 0)
            {
