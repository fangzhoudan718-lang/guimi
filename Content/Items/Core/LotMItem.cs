...
namespace zhashi.Content
{
    public abstract class LotMItem : ModItem
    {
        ...
        public override void ModifyTooltips(List<TooltipLine> tooltips)
        {
            if (RequiredSequence > 0 && Pathway != "None")
            {
                ...
                switch (Pathway)
                {
                    ...
                    case "Door": pathName = "学徒/门"; break;
                    case "BlackEmperor": pathName = "黑皇帝"; break;
                }
                string requirement = Pathway == "Door" && RequiredSequence == 10
                    ? "仅尚未选择途径的凡人可饮用"
                    : $"需要 {pathName} 途径 序列 {RequiredSequence}";
                tooltips.Add(new TooltipLine(Mod, "SequenceReq", $"[c/{colorHex}:{requirement} ({statusText})]"));
            }
        }
        ...
        private bool CheckSequenceRequirement(LotMPlayer p)
        {
            ...
            // 门途径必须严格按9→1依次晋升。使用基础序列而不是临时序列，
            // 防止高序列误饮低序列魔药导致降级，也防止借助临时状态越级。
            if (Pathway == "Door")
                return p.baseDoorSequence == RequiredSequence &&
                       (RequiredSequence != 10 || !p.IsBeyonder);
            // 黑皇帝同样必须严格按 9→1 依次晋升，判定基础序列而不是临时序列。
            if (Pathway == "BlackEmperor")
                return p.baseBlackEmperorSequence == RequiredSequence &&
                       (RequiredSequence != 10 || !p.IsBeyonder);
            return false;
        }
    }
