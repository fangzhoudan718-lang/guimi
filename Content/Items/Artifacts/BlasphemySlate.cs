using System.Collections.Generic;
using Microsoft.Xna.Framework;
using Terraria;
using Terraria.ID;
using Terraria.ModLoader;
using Terraria.ModLoader.IO;
using zhashi.Content;

namespace zhashi.Content.Items
{
    public class BlasphemySlate : ModItem
    {
        public int recipeIndex = -1;

        // 【修改】增加了 string ritual 参数 (第5个参数)
        // 格式: (等级, 途径名, 序列名, 配方材料, 晋升仪式说明, 颜色)
        private static readonly List<(int level, string pathway, string name, string content, string ritual, Color color)> RecipeDatabase = new()
        {
            // ==========================================
            //               愚者途径 (紫色)
            // ==========================================
            (9, "愚者途径", "序列9 占卜家", "水瓶 + 黑墨水 + 坠落之星 + 太阳花 + 闪耀根 + 死亡草", "直接服用魔药。", Color.MediumPurple),
            (8, "愚者途径", "序列8 小丑", "水瓶 + 毒刺 + 丛林玫瑰 + 闪耀根 + 向日葵 + 月光草 + 死亡草", "直接服用魔药。", Color.MediumPurple),
            (7, "愚者途径", "序列7 魔术师", "水瓶 + 木材(100) + 黑墨水(10) + 蓝宝石(20) + 死亡草(10)", "直接服用魔药。", Color.MediumPurple),
            (6, "愚者途径", "序列6 无面人", "水瓶 + 腐肉/脊椎(5) + 骨头(10) + 死亡草 + 丛林孢子(3) + 鲨鱼鳍", "直接服用魔药。", Color.MediumPurple),
            (5, "愚者途径", "序列5 秘偶大师", "水瓶 + 飞翔之魂(5) + 神圣锭(5) + 红木(5) + 骨头(10) + 晶状体(2)", "【仪式】：必须在海洋。", Color.MediumPurple),
            (4, "愚者途径", "序列4 诡法师", "水瓶 + 力量之魂(10) + 灵气(10) + 红木(5) + 藤蔓 + 丝绸(5)", "【仪式】：1. 谋杀半神：必须已经击败世纪之花。2. 观众：周围必须有至少 10 个 NPC (城镇NPC或敌人均可)。", Color.MediumPurple),
            (3, "愚者途径", "序列3 古代学者", "水瓶 + 日耀碎片(10) + 星云碎片(10) + 寒霜核(3) + 灵气(10) + 书(5)", "【仪式】：脱离现实 (处于太空层 )。", Color.MediumPurple),
            (2, "愚者途径", "序列2 奇迹师", "水瓶 + 夜明矿(20) + 灵气(20) + 金表 + 坠落之星(10)", "【仪式】：在具有历史气息的地方 (地牢)。", Color.MediumPurple),
            (1, "愚者途径", "序列1 诡秘侍者", "水瓶 + 夜明锭(20) + 灵气(5) + 飞翔/光明/暗影之魂(5) + 独角兽角 + 精灵尘(10) + 水晶碎块(5) + 咒火(5) + 灵液(5)", "【诡秘仪式】：在序列2的状态下，使用灵体之线将10位城镇居民转化为秘偶。", Color.MediumPurple),

            // ==========================================
            //               巨人途径 (金色)
            // ==========================================
            (9, "巨人途径", "序列9 战士", "水瓶 + 太阳花 + 铁/铅锭(3)", "直接服用魔药。", Color.Gold),
            (8, "巨人途径", "序列8 格斗家", "水瓶 + 魔矿/猩红锭(5) + 凝胶(50) + 丛林孢子(3)", "直接服用魔药。", Color.Gold),
            (7, "巨人途径", "序列7 武器大师", "水瓶 + 暗影鳞片/组织样本(10) + 黑曜石(5) + 毒刺(3)", "直接服用魔药。", Color.Gold),
            (6, "巨人途径", "序列6 黎明骑士", "水瓶 + 神圣锭(5) + 光明之魂(10) + 太阳花(5)", "直接服用魔药。", Color.Gold),
            (5, "巨人途径", "序列5 守护者", "水瓶 + 叶绿锭(5) + 生命果(3) + 铁皮药水(5)", "【仪式】：在序列6的状态下，在城镇居民附近累计承受1000点伤害，证明守护之心。", Color.Gold),
            (4, "巨人途径", "序列4 猎魔者", "水瓶 + 甲虫壳(5) + 灵气(10) + 暗影之魂(15)", "【仪式】：在序列5的状态下，亲手击杀10只红魔鬼。", Color.Gold),
            (3, "巨人途径", "序列3 银骑士", "水瓶 + 霜月纪念章(3) + 南瓜月纪念章(2) + 阴森木(50)", "【仪式】: 通关霜月与南瓜月事件，证明你的杀戮能力。", Color.Gold),
            (2, "巨人途径", "序列2 荣耀者", "水瓶 + 夜明锭(10) + 日耀碎片(10) + 星云碎片(5)", "【仪式】: 猎杀一位天使或同等强大的生物 (月亮领主)。", Color.Gold),
            (1, "巨人途径", "序列1 神明之手", "水瓶 + 神之手非凡特性 + 生命果（5) + 灵气(20)", "直接服用魔药。", Color.Gold),

            // ==========================================
            //               红祭司途径 (红色)
            // ==========================================
            (9, "红祭司途径", "序列9 猎人", "水瓶 + 麦芽酒 + 太阳花 + 凝胶(5)", "直接服用魔药。", Color.IndianRed),
            (8, "红祭司途径", "序列8 挑衅者", "水瓶 + 麦芽酒 + 藤蔓(2) + 黑曜石(5) + 水叶草(2)", "直接服用魔药。", Color.IndianRed),
            (7, "红祭司途径", "序列7 纵火家", "水瓶 + 狱石锭(5) + 火焰花(3) + 爆炸粉(10)", "直接服用魔药。", Color.IndianRed),
            (6, "红祭司途径", "序列6 阴谋家", "水瓶 + 蜘蛛牙(5) + 蚁狮上颚(5) + 琥珀(3) + 橡果(5)", "直接服用魔药。", Color.IndianRed),
            (5, "红祭司途径", "序列5 收割者", "水瓶 + 神圣锭(10) + 恐惧之魂(5) + 暗影之魂(10) + 鲨鱼鳍(5)", "直接服用魔药。", Color.IndianRed),
            (4, "红祭司途径", "序列4 铁血骑士", "水瓶 + 甲虫壳(10) + 狱石锭(10) + 力量之魂(5) + 视域之魂(5)", "【仪式】: 在至少有5名随从的情况下，无死亡累计击杀100个敌人。", Color.IndianRed),
            (3, "红祭司途径", "序列3 战争主教", "水瓶 + 幽灵锭(10) + 蘑菇矿锭(10) + 电池", "【仪式】: 击败拜月教邪教徒，开启最终的战争。", Color.IndianRed),
            (2, "红祭司途径", "序列2 天气术士", "水瓶 + 四柱碎片各(10)", "【仪式】: 连续使用十次天气符文。", Color.IndianRed),
            (1, "红祭司途径", "序列1 征服者", "需[征服者特性] (巫毒娃娃x2+铂金币(5)+各类NPC掉落物)", "【征服仪式】：在序列2的状态下，使用征服者特性开启'世界静默'，并清剿所有敌对生物。", Color.IndianRed),

            // ==========================================
            //               月亮途径 (深红)
            // ==========================================
            (9, "月亮途径", "序列9 药师", "水瓶 + 毒刺(3) + 荧光棒(7) + 死亡草", "直接服用魔药。", Color.Crimson),
            (8, "月亮途径", "序列8 驯兽师", "水瓶 + 蓝宝石 + 丛林孢子(3) + 太阳花 + 弱效治疗药水 + 铁/铅锭", "直接服用魔药。", Color.Crimson),
            (7, "月亮途径", "序列7 吸血鬼", "水瓶 + 暗影鳞片/组织样本 + 骨头(10) + 水蜡烛 + 月光草(3) + 死亡草(3)", "直接服用魔药。", Color.Crimson),
            (6, "月亮途径", "序列6 魔药教授", "水瓶 + 生命水晶 + 灵液（咒火）(5) + 鲨鱼鳍(3)", "直接服用魔药。", Color.Crimson),
            (5, "月亮途径", "序列5 深红学者", "水瓶 + 神圣锭(5) + 暗影之魂(10) + 光明之魂(5) + 独角兽角", "直接服用魔药。", Color.Crimson),
            (4, "月亮途径", "序列4 巫王", "水瓶 + 灵气(10) + 破蝙蝠之翼 + 暗黑碎片(3) + 暗影之魂(15) + 月亮石", "【仪式】：处在满月的照耀下。", Color.Crimson),
            (3, "月亮途径", "序列3 召唤大师", "水瓶 + [真名拓本] + 灵气(10) + 幽灵锭(5)", "【仪式】：需先使用'灵界低语'成功解析出'真名拓本'作为核心材料。", Color.Crimson),
            (2, "月亮途径", "序列2 创生者", "水瓶 + 四柱碎片各(10) + 夜明锭(5)", "【仪式】：在彻底死亡之前服用。", Color.Crimson),
            (1, "月亮途径", "序列1 美神", "水瓶 + 夜明锭(20) + 四柱碎片各(10) + 生命水晶(5)", "【美神仪式】：在岩石层的床上服用 (注意：服用后会强制转变为女性)。", Color.Crimson),

            // ==========================================
            //               错误途径 (深蓝)
            // ==========================================
            (9, "错误途径", "序列9 偷盗者", "水瓶 + 毒刺 + 粉凝胶 + 蓝宝石", "直接服用魔药。", Color.DarkBlue),
            (8, "错误途径", "序列8 诈骗师", "水瓶 + 蠕虫 + 丛林孢子 + 晶状体 + 蓝宝石", "直接服用魔药。", Color.DarkBlue),
            (7, "错误途径", "序列7 解密学者", "水瓶 + 蚁狮上颚(5) + 附魔夜行者 + 钻石 + 书", "直接服用魔药。", Color.DarkBlue),
            (6, "错误途径", "序列6 盗火人", "水瓶 + 水晶碎块 + 麦芽酒 + 黄玉（10） + 活火块", "直接服用魔药。", Color.DarkBlue),
            (5, "错误途径", "序列5 窃梦家", "水瓶 + 黑晶状体 + 暗影之魂(5) + 蓝宝石 + 月光草(3)", "直接服用魔药。", Color.DarkBlue),
            (4, "错误途径", "序列4 寄生者", "水瓶 + 灵气(5) + 松露虫 + 紫水晶 + 暗影之魂(5) + 葡萄", "【仪式】：在序列5的状态下，通过攻击或窃取累积9次'命运的馈赠'。", Color.DarkBlue),
            (3, "错误途径", "序列3 欺瞒导师", "水瓶 + 灵气(10) + 腐香囊 + 毒药瓶(5) + 丛林孢子(20) + 金锭(5)", "【仪式】：在序列4的状态下，通过混乱/误导致死9个敌人。", Color.DarkBlue),
            (2, "错误途径", "序列2 命运木马", "水瓶 + 灵气(10) + 光明之魂(10) + 暗影之魂(10) + 松露虫 + 附魔夜行者 + 圣水(5)", "【仪式】：在序列3的状态下，完全寄生控制一名城镇居民长达5分钟。", Color.DarkBlue),
            (1, "错误途径", "序列1 时之虫", "水瓶 + 夜明锭(5) + 星尘碎片(10) + 1秒计时器 + 书 + 灵气(5)", "【时光仪式】：在序列2的状态下，于拥有3名以上居民的城镇中维持'欺瞒领域'达7分钟。", Color.DarkBlue),

            // ==========================================
            //                太阳途径 (橙色)
            // ==========================================
            (9, "太阳途径", "序列9 歌颂者", "水瓶 + 向日葵 + 羽毛 + 麦芽酒 + 太阳花 + 月光草", "直接服用魔药。", Color.Orange),
            (8, "太阳途径", "序列8 祈光人", "水瓶 + 坠落之星(3) + 火焰花 + 岩浆石 + 麦芽酒 + 向日葵 + 水叶草 + 金锭", "直接服用魔药。", Color.Orange),
            (7, "太阳途径", "序列7 太阳神官", "水瓶 + 火焰花(3) + 狱石锭(5) + 蜂蜜块(5) + 熔岩桶 + 金锭(2)", "直接服用魔药。", Color.Orange),
            (6, "太阳途径", "序列6 公证人", "水瓶 + 神圣锭(5) + 飞翔之魂(3) + 妖精尘(10) + 向日葵(2)", "直接服用魔药。", Color.Orange),
            (5, "太阳途径", "序列5 光之祭司", "水瓶 + 太阳石 + 钻石(5) + 神圣锭(10) + 光明之魂(10) + 妖精尘(20)", "【仪式】：在序列6的状态下，累计净化(击杀)大量不死生物(僵尸/骷髅等)。", Color.Orange),
            (4, "太阳途径", "序列4 无暗者", "水瓶 + 太阳石板(5) + 红玉(5) + 黄玉(5) + 熔岩桶 + 视域之魂(5)", "【仪式】：1. 累计审判(击杀)一定数量的Boss或强敌。 2. 背包中需持有[无暗十字]。", Color.Orange),
            (3, "太阳途径", "序列3 正义导师", "水瓶 + 日耀碎片(15) + 灵气(10) + 生命果(5) + 孢子囊 + 神圣锭(20)", "【仪式】：在白天的纯净状态下(无流血/中毒)，手持[复仇者徽章]作为正义的证明服用。", Color.Orange),
            (2, "太阳途径", "序列2 逐光者", "水瓶 + 日耀碎片(20) + 光明之魂(20) + 生命果(5) + 灵气(10) + 书", "【仪式】：在烈日当空时(白天)，携带[破晓]或[日耀喷发剑]作为道标，并保持满血状态服用。", Color.Orange),
            (1, "太阳途径", "序列1 纯白天使", "水瓶 + 日耀碎片(50) + 七彩草蛉 + 神圣锭(50) + 夜明锭(50) + 蜥蜴电池(5) + 光明之魂(50) + 灵气(30)", "【纯白仪式】：在正午(12:00-14:00)，城镇居民达到15人，并持有[泰拉刃/终极棱镜/彩虹猫之刃]之一服用。", Color.Orange),

            // ==========================================
            //               魔女途径 (粉色)
            // ==========================================
            (9, "魔女途径", "序列9 刺客", "水瓶 + 镜面鱼 + 毒刺(2) + 死亡草 + 水叶草", "直接服用魔药。", Color.HotPink),
            (8, "魔女途径", "序列8 教唆者", "水瓶 + 骨头(5) + 腐肉/脊椎(3) + 暗影鳞片/组织样本(2) + 水叶草", "直接服用魔药。", Color.HotPink),
            (7, "魔女途径", "序列7 女巫", "水瓶 + 黑曜石鱼 + 水晶碎块(3) + 太阳花 + 暗影鳞片/组织样本(3) + 水叶草(2)", "【注意】：服用后会改变性别。", Color.HotPink),
            (6, "魔女途径", "序列6 欢愉魔女", "水瓶 + 晶状体(2) + 蜘蛛牙 + 破布 + 臭味药水 + 暗黑碎片", "【仪式】：需将 护士/树妖/向导/渔夫 中任一NPC的好感度提升至100。", Color.HotPink),
            (5, "魔女途径", "序列5 痛苦魔女", "水瓶 + 毒刺(10) + 椎骨(50) + 藤蔓(10)", "【仪式】：需持续忍受烈火灼烧1分钟 (累计时间)。", Color.HotPink),
            (4, "魔女途径", "序列4 绝望魔女", "水瓶 + 毒液瓶 + 灵气(3) + 水晶碎块(5) + 丛林孢子(5) + 腐肉(5)", "【仪式】：在日食期间累计击杀50个敌人。", Color.HotPink),
            (3, "魔女途径", "序列3 不老魔女", "水瓶 + 灵气(5) + 神圣锭(5) + 魔镜 + 生命果 + 水晶碎块(10)", "【仪式】：召唤镜面分身(按Z键)，并走向分身与其重叠融合。", Color.HotPink),
            (2, "魔女途径", "序列2 灾难魔女", "星云碎片(10) + 日耀碎片(10) + 雨云(50) + 叶绿锭(5) + 灵气(10) + 咒火(20)", "【仪式】：处于灾难天气(雨/雪/沙暴)且周围有BOSS或大规模入侵事件。", Color.HotPink),
            (1, "魔女途径", "序列1 末日魔女", "夜明矿(100) + 四柱碎片各(20) + 夜明锭(20) + 水瓶", "【末日仪式】：在月亮领主降临且自身濒死(血量<30%)时服用。", Color.HotPink),

            // ==========================================
            //               命运途径 (紫金色)
            // ==========================================
            (9, "命运途径", "序列9 怪物", "水瓶 + 蓝史莱姆 + 太阳花 + 蓝宝石 + 黑墨水", "直接服用魔药。", Color.MediumOrchid),
            (8, "命运途径", "序列8 机器", "水瓶 + 黄铜手表/银怀表 + 黄玉(3) + 齿轮(10) + 钉子(10)", "直接服用魔药。", Color.MediumOrchid),
            (7, "命运途径", "序列7 幸运儿", "水瓶 + 四叶草徽章 + 金锭(10) + 太阳花 + 麦芽酒", "直接服用魔药。", Color.MediumOrchid),
            (6, "命运途径", "序列6 灾祸祭司", "水瓶 + 神圣锭(10) + 视域之魂(5) + 黑曜石(10) + 太阳石", "直接服用魔药。", Color.MediumOrchid),
            (5, "命运途径", "序列5 赢家", "水瓶 + 幸运硬币 + 神圣锭(10) + 金锭(20) + 钻石(2)", "直接服用魔药。", Color.MediumOrchid),
            (4, "命运途径", "序列4 厄运法师", "水瓶 + 神圣锭(10) + 恐惧之魂(5) + 视域之魂(5) + 力量之魂(5) + 骨头(30)", "【仪式】：佩戴【厄运之骰】(灰色四叶草)击败机械骷髅王。", Color.MediumOrchid),
            (3, "命运途径", "序列3 怪人", "水瓶 + 神圣锭(15) + 灵气(10) + 三种之魂各(5) + 生命果", "【仪式】：在序列4状态下，3次于<10%血量时存活并恢复至60%+生命，让命运为你倾斜。", Color.MediumOrchid),
            (2, "命运途径", "序列2 先知", "水瓶 + 月光锭(10) + 星云碎片(10) + 漩涡碎片(10) + 灵气(15) + 生命果(3) + 光明/暗影之魂各(10)", "【仪式】：在序列3状态下，让\"不可定数\"成功触发5次，五次见证命运拒绝死亡。", Color.MediumOrchid),
            (1, "命运途径", "序列1 巨蛇", "水瓶 + 月光锭(25) + 星云/漩涡/日耀/星尘碎片各(15) + 灵气(30) + 生命果(5) + 神圣锭(30)", "【仪式】：在序列2状态下，触发命运启示5次,且击败月亮领主。", Color.MediumOrchid),

              // ==========================================
            //               门途径
            // ==========================================
            (9, "学徒/门途径", "序列9 学徒", "水瓶 + 蠕虫 + 紫水晶 + 死亡草 + 泥块(5)", "直接服用；仅尚未选择途径的凡人可饮用。材料映射吞宝石蠕虫、幻影水晶、尸生花与灵界土壤。", new Color(232, 200, 120)),
            (8, "学徒/门途径", "序列8 戏法大师", "水瓶 + 鲨鱼鳍 + 蛛网(20) + 火焰花 + 毒刺(3)", "直接服用；仅序列9学徒可饮用。材料映射深海枪鱼、线球草、红栗花与食灵者胃袋。", new Color(232, 200, 120)),
            (7, "学徒/门途径", "序列7 占星人", "水瓶 + 陨石锭(5) + 黑墨水(2) + 钻石 + 麦芽酒", "直接服用；仅序列8戏法大师可饮用。材料映射陨星水晶、章鱼血液结晶与烈酒。", new Color(232, 200, 120)),
            (6, "学徒/门途径", "序列6 记录官", "水瓶 + 书(3) + 黑晶状体 + 骨头(20) + 光明/暗影之魂各(5)", "直接服用；仅序列7占星人可饮用。材料映射陈旧日记、阿斯曼之脑与古老怨灵诅咒物。", new Color(232, 200, 120)),
            (5, "学徒/门途径", "序列5 旅行家", "水瓶 + 蠕虫 + 寒霜核 + 神圣锭(10) + 黑墨水(3) + 飞翔之魂(10)", "直接服用；仅序列6记录官可饮用。材料映射魔虫、无影魔狼心脏、幽灵残留与星图。", new Color(232, 200, 120)),
            (4, "学徒/门途径", "序列4 秘法师", "水瓶 + 水晶球 + 水晶碎块(20) + 灵气(10) + 飞翔之魂(10) + 神圣锭(10)", "【仪式】：将敌对Boss削弱至20%生命以下，靠近后尝试使用秘法师魔药启动封印；仪式完成后再次饮用晋升。", new Color(216, 160, 255)),
            (3, "学徒/门途径", "序列3 漫游者", "水瓶 + 灵气(20) + 星云/漩涡碎片各(10) + 火星传导板(30) + 飞翔之魂(20)", "【仪式】：分别进入灵界、血月高空与任意四柱影响区，记录三份星界信息衍生的危险场景。", new Color(168, 120, 255)),
            (2, "学徒/门途径", "序列2 旅法师", "水瓶 + 夜明锭(15) + 四柱碎片各(15) + 灵气(20)", "【仪式】：在太空层或异度空间击败九种不同Boss，在星球之外留下九段传说。", new Color(128, 200, 255)),
            (1, "学徒/门途径", "序列1 星之匙", "水瓶 + 夜明锭(30) + 天界符(1) + 指南针(1) + 四柱碎片各(20)", "【仪式】：在太空层击败月亮领主，与不断旋转并发出信号的沉重星体建立神秘学联系。", new Color(255, 240, 160)),

            // ==========================================
            //               黑皇帝途径
            // ==========================================
            (9, "黑皇帝途径", "序列9 律师", "水瓶 + 书 + 羽毛(10) + 木材(20) + 凝胶(10)", "直接服用；仅尚未选择途径的凡人可饮用。材料映射智慧果与迷宫鹦鹉的舌头。", new Color(176, 122, 200)),
            (8, "黑皇帝途径", "序列8 野蛮人", "水瓶 + 铁/铅锭(15) + 凝胶(50) + 丛林孢子(5) + 骨头(10)", "直接服用；仅序列9律师可饮用。材料映射狂化草与大地犀牛的实心独角结晶。", new Color(176, 122, 200)),
            (7, "黑皇帝途径", "序列7 贿赂者", "水瓶 + 金币(5) + 黄玉(3) + 麦芽酒(5) + 死亡草(5)", "直接服用；仅序列8野蛮人可以饮用。材料映射哭泣婴儿花与怪脸大麻结晶。", new Color(176, 122, 200)),
            (6, "黑皇帝途径", "序列6 腐化男爵", "水瓶 + 腐肉/脊椎(15) + 蜂巢(10) + 暗影鳞片/组织样本(5) + 蛛网(30)", "直接服用；仅序列7贿赂者可以饮用。材料映射腐烂尸花与人脸狒狒的鼻子。", new Color(176, 122, 200)),
            (5, "黑皇帝途径", "序列5 混乱导师", "水瓶 + 陨石锭(12) + 狱岩石(20) + 精灵尘(10) + 金/铂金锭(5) + 黑曜石(15)", "【仪式】：连着三个夜晚，每夜在洞穴层或更深处了结足够多的敌人；城镇里至少要住着三位居民，夜里别对居民动手。", new Color(154, 106, 176)),
            (4, "黑皇帝途径", "序列4 堕落伯爵", "水瓶 + 神圣锭(15) + 灵质(20) + 暗影之魂(15) + 铂金币(3)", "【仪式】：拉拢七位不同的城镇居民，再在城里把一条律令维持到自然结束，让政策落地。", new Color(139, 95, 176)),
            (3, "黑皇帝途径", "序列3 狂乱法师", "水瓶 + 星旋碎片(10) + 星云碎片(10) + 火星管道镀层(30) + 飞翔之魂(20) + 灵质(20)", "【仪式】：在三场不同的 Boss 战里扬起狂乱，并且活着打完；同一个 Boss 只算一次。", new Color(160, 120, 208)),
            (2, "黑皇帝途径", "序列2 熵之公爵", "水瓶 + 月球锭(15) + 四柱碎片各(15) + 灵质(20) + 钻石(1)", "【仪式】：在乱世之夜（血月 / 入侵 / 霜月 / 南瓜月 / 日食），站在城镇里引爆一次满层的熵——熵在交战中每三秒叠一层，只涨不掉。", new Color(186, 150, 224)),
            (1, "黑皇帝途径", "序列1 弑序亲王", "水瓶 + 月球锭(30) + 天界符(1) + 罗盘(1) + 四柱碎片各(20) + 铂金币(10)", "【仪式】：以自身的秩序取代原本的秩序——在三个不同的昼夜，空手站在城镇里，把一条律令维持到自然结束；每个昼夜只记一次。", new Color(224, 192, 112)),

        };

        public override void SetDefaults()
        {
            Item.width = 30;
            Item.height = 30;
            Item.maxStack = 1;
            Item.value = Item.sellPrice(gold: 1);
            Item.rare = ItemRarityID.LightRed;
            Item.useStyle = ItemUseStyleID.HoldUp;
            Item.useTime = 20;
            Item.useAnimation = 20;
            Item.consumable = true;
        }

        public override void SaveData(TagCompound tag) => tag["SlateRecipeIndex"] = recipeIndex;
        public override void LoadData(TagCompound tag) => recipeIndex = tag.GetInt("SlateRecipeIndex");

        public override void UpdateInventory(Player player)
        {
            if (recipeIndex == -1)
            {
                recipeIndex = Main.rand.Next(RecipeDatabase.Count);
            }
        }

        public override void ModifyTooltips(List<TooltipLine> tooltips)
        {
            // =========================================================
            // 1. 处理固定描述 (解决重复显示问题，同时保留颜色)
            // =========================================================

            // 遍历所有提示行 (包含系统自动从hjson加载的)
            foreach (TooltipLine line in tooltips)
            {
                // 如果检测到是那句“背景描述”，将其染成灰色
                if (line.Text.Contains("这块古老的石板"))
                {
                    line.OverrideColor = new Color(150, 150, 150);
                }

                // 如果检测到是那句“用法说明”，将其染成橙色
                // (注意：hjson中可能已经有颜色代码，但这行代码确保它一定是橙色)
                if (line.Text.Contains("【消耗品】"))
                {
                    line.OverrideColor = Color.Orange;
                }
            }

            // =========================================================
            // 2. 处理动态逻辑 (混沌状态 / 配方 / 仪式)
            // =========================================================
            if (recipeIndex == -1)
            {
                // 混沌状态 (还在箱子里或刚拿出来)
                int tempIndex = Main.rand.Next(RecipeDatabase.Count);
                var tempData = RecipeDatabase[tempIndex];

                TooltipLine chaosLine = new TooltipLine(Mod, "Chaos",
                    $"[混沌状态] 拾取以观测...\n>>> {tempData.name} ???");
                chaosLine.OverrideColor = Color.Lerp(Color.Gray, tempData.color, 0.5f);
                tooltips.Add(chaosLine);
            }
            else if (recipeIndex >= 0 && recipeIndex < RecipeDatabase.Count)
            {
                // 正常显示配方状态
                var data = RecipeDatabase[recipeIndex];

                // 标题
                TooltipLine titleLine = new TooltipLine(Mod, "RecipeTitle", $"\n>>> {data.pathway}: {data.name} <<<");
                titleLine.OverrideColor = data.color;
                tooltips.Add(titleLine);

                // 配方内容
                string wrappedContent = GetWrappedText(data.content, 40);
                TooltipLine contentLine = new TooltipLine(Mod, "RecipeContent", $"[合成配方]:\n{wrappedContent}");
                contentLine.OverrideColor = Color.White;
                tooltips.Add(contentLine);

                // 仪式说明
                string wrappedRitual = GetWrappedText(data.ritual, 40);
                TooltipLine ritualLine = new TooltipLine(Mod, "RitualContent", $"[晋升仪式]:\n{wrappedRitual}");
                ritualLine.OverrideColor = new Color(255, 255, 200); // 淡黄色
                tooltips.Add(ritualLine);
            }
        }

        // 自动换行辅助函数
        private string GetWrappedText(string text, int lineLength)
        {
            if (string.IsNullOrEmpty(text)) return "";
            if (text.Length <= lineLength) return text;

            System.Text.StringBuilder sb = new System.Text.StringBuilder();
            int currentLineLen = 0;

            // 尝试按 '+' 分割 (针对配方)
            string[] parts = text.Contains("+") ? text.Split('+') : new string[] { text };

            // 如果没有 '+' (比如仪式说明是纯文本)，则按长度强制换行
            if (parts.Length == 1)
            {
                for (int i = 0; i < text.Length; i++)
                {
                    sb.Append(text[i]);
                    if ((i + 1) % lineLength == 0) sb.Append("\n");
                }
                return sb.ToString();
            }

            // 按配方逻辑换行
            for (int i = 0; i < parts.Length; i++)
            {
                string part = parts[i].Trim();
                string toAdd = (i == 0 ? "" : " + ") + part;

                if (currentLineLen + toAdd.Length > lineLength)
                {
                    sb.Append("\n");
                    sb.Append(toAdd.TrimStart('+').Trim());
                    currentLineLen = toAdd.Length;
                }
                else
                {
                    sb.Append(toAdd);
                    currentLineLen += toAdd.Length;
                }
            }
            return sb.ToString();
        }
    }
}
